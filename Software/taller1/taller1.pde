...
void draw() {
  ...
  if (option==2) {
    float[][]matriz = {{-1, -1, -1}, {-1, 8, -1}, {-1, -1, -1}};
    convolucion(matriz);
  }
  if (option==3) {
    float[][]matriz = {{0, -1, 0}, {-1, 5, -1}, {0, -1, 0}};
    convolucion(matriz);
  }
  if (option==4) {
    float[][]matriz = {{-2, -1, 0}, {-1, 2, 1}, {0, 1, 2}};
    convolucion(matriz);
  }
  ...
  if (select) {
    ...
    for (int i = 0; i < myMovie.width; i+=resolution) {
      for (int j = 0; j < myMovie.height; j+=resolution) {
        ...
        fill(c);
        text(chars[int(brightness(c)/25.5)], 510 +i, 30 + j);
      }
    }
...
void asciify() {
  ...
  for (int y = 0; y < img.height; y += resolution) {
    for (int x = 0; x < img.width; x += resolution) {
      color pixel = img.pixels[y * pg.width + x];
      text(ascii[int(brightness(pixel))], 510 +x, 30 + y);
    }
  }
...
void botones() {
  ...
  if (mouseX > x && mouseX < x+w && mouseY > y+40 && mouseY < y+h+40&& mousePressed==true) {
    select=false;
  }
  ...
  if (mouseX > xx && mouseX < xx+w && mouseY > yy && mouseY < yy+h&& mousePressed==true) {
    option=1;
  }
  ...
  if (mouseX > xx+150 && mouseX < xx+w+150 && mouseY > yy && mouseY < yy+h&& mousePressed==true) {
    option=2;
  }
  ...
  if (mouseX > xx+300 && mouseX < xx+w+300 && mouseY > yy && mouseY < yy+h&& mousePressed==true) {
    option=3;
  }
  ...
  if (mouseX > xx+450 && mouseX < xx+w+450 && mouseY > yy && mouseY < yy+h&& mousePressed==true) {
    option=4;
  }
  ...
  if (mouseX > xx+600 && mouseX < xx+w+600 && mouseY > yy && mouseY < yy+h&& mousePressed==true) {
    option=5;
  }
  ...
  if (mouseX > xx+850 && mouseX < xx+w+850 && mouseY > yy && mouseY < yy+h&& mousePressed==true) {
    option=7;
  }
